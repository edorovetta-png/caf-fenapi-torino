...
CREATE POLICY "Admins can update profiles"
  ON profiles FOR UPDATE USING (auth.user_role() = 'admin');

-- PRODUCTS
CREATE POLICY "Anyone authenticated can read active products"
  ON products FOR SELECT USING (is_active = true OR auth.user_role() = 'admin');
CREATE POLICY "Admins can insert products"
  ON products FOR INSERT WITH CHECK (auth.user_role() = 'admin');
CREATE POLICY "Admins can update products"
  ON products FOR UPDATE USING (auth.user_role() = 'admin');
CREATE POLICY "Admins can delete products"
  ON products FOR DELETE USING (auth.user_role() = 'admin');

-- CUSTOMERS
CREATE POLICY "Anyone authenticated can read customers"
  ON customers FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY "Admins can insert customers"
  ON customers FOR INSERT WITH CHECK (auth.user_role() = 'admin');
CREATE POLICY "Admins can update customers"
  ON customers FOR UPDATE USING (auth.user_role() = 'admin');
CREATE POLICY "Admins can delete customers"
  ON customers FOR DELETE USING (auth.user_role() = 'admin');
...
CREATE POLICY "Admins can do anything with order items"
  ON order_items FOR ALL USING (auth.user_role() = 'admin');
