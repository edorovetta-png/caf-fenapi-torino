...
> Ultimo aggiornamento: 2026-09-09 sera (**Sezione BANDI ENFIP — `frontend/bandi/`, commit su `feat/fenapi-enfip-page`, push a cura dell'utente**. Richiesta: voce 'Bandi' in risalto nel menu laterale di `enfip.html` → pagina di scelta categoria → pagina categoria con le locandine. Fatto: (1) `enfip.html`: prima voce della sidebar `Bandi e corsi in partenza` con classe `.sidebar-highlight` (sfondo oro `--accent-warm`, CSS inline nel critical CSS) → `bandi/index.html`; link anche nella sezione Contatti. Recapiti nell'hero resi più evidenti (pill bianche, telefono in oro, commit `1fd3c17`). (2) `bandi/index.html`: griglia 6 categorie (agricoltura, ficp, apprendisti, sicurezza, fitosanitari, fondo-conoscenza) con badge conteggio bandi aperti calcolato in JS. (3) `bandi/<slug>.html` ×6: sezione 'Bandi aperti' + sezione 'Bandi conclusi' SEMPRE visibile sotto (storico per i controlli regionali; entrambe le sezioni hanno uno stato vuoto) + lightbox per la locandina. (4) **Dati in `bandi/bandi-data.js`** (`window.ENFIP_BANDI = [...]`, un oggetto per bando: id, categoria, titolo, descrizione, sede, pubblicato, scadenza, locandina, pdf, stato) — nessun fetch, nessun build: per pubblicare un bando si aggiunge un oggetto e si carica l'immagine in `bandi/locandine/`. Un bando passa da solo in archivio quando `scadenza` < oggi, oppure con `stato: 'chiuso'`. Rendering in `bandi/bandi.js`, stili in `bandi/bandi.css` (su mobile prima i contenuti, poi la sidebar). (5) Le 7 pagine sono generate da **`execution/generate_bandi_pages.py`** partendo dal template di `enfip.html` (header/footer/critical CSS con path `../`): rilanciarlo dopo modifiche a enfip.html o alle categorie, NON editare le pagine bandi a mano. (6) `sitemap.xml` +7 URL, `llms.txt` +1 riga. Schema: CollectionPage + BreadcrumbList per pagina. Verificato con screenshot desktop/mobile (con 3 bandi di prova, poi rimossi: la lista è vuota → stato 'Nessun bando aperto'). **Next**: l'utente manda le prime locandine → inserirle in `bandi-data.js` + `bandi/locandine/`; valutare voce 'Bandi' anche nell'hero di enfip.html.)
> Ultimo aggiornamento precedente: 2026-09-09 (**Nuova pagina ENFIP interna al sito — `frontend/enfip.html` — COMMITTATA E IN PRODUZIONE**. Commit `3574022` su branch `feat/fenapi-enfip-page` (creato da `origin/main`, non dal `main` locale che è ~198 commit avanti con lavoro piano-finanziario non pushato), pushato dall'utente dal Terminale e fast-forward su `origin/main` (`90c83f1..3574022`); Vercel auto-deploy verificato live: `/enfip.html` HTTP 200 byte-identico al file locale, 4 JSON-LD validi, homepage e sitemap live puntano a `enfip.html`. **Nota git**: `main` locale NON è stato aggiornato — per riallinearlo `git checkout main && git merge feat/fenapi-enfip-page` (nessun conflitto atteso). Prima il link ENFIP (card 'Il Gruppo FENAPI' + footer di index.html) puntava a `fenapigroup.it/enfip/`, pagina esterna vuota. Creata `enfip.html` alla root del sito (URL canonico `https://www.fenapipiemonte.org/enfip.html`) con lo stesso template di `chi-siamo.html` (critical CSS inline, `styles.css` + `service-page.css`, hero, sidebar 'In questa pagina', `.content-section`, footer). Contenuti dal documento dell'utente `SITO ENFIP.docx`: chi siamo, storia (fondazione 21/03/2003 da FENAPI), 4 aree formative (linguistica/digitale, amministrazione, sicurezza-HACCP, agricoltura — la quarta è dedotta dalla sezione 'Agricoli' del doc), apprendistato I/II livello + ricollocazione, FICP (Buoni Formazione 70% Regione), formazione agricola CSR/FEASR (SRH03), patentino fitosanitari (SOLO Acqui Terme), sicurezza D.Lgs. 81/2008 (lavoratori, RSPP, RLS, preposti, macchine/attrezzature con 3 step), Fondo Conoscenza (0,30%, codice adesione UNIEMENS FCON, 4 step) e contatti. **Contatti ENFIP** usati ovunque nella pagina (floating CTA, sidebar, CTA finale): tel 011 799999 int. 1, info@enfip.eu, FB `facebook.com/enfip`; sede Via Sagra di San Michele 53/55 Torino (fonte: scheda accreditamento INAPP `inapp.gov.it/accreditamento/enfip-piemonte-2858/`; una sede Rivoli Via Pavia 11/B trovata online NON inserita per scelta dell'utente). **SEO**: title/description/keywords su 'ENFIP', 'ENFIP Piemonte', 'E.N.F.I.P.'; canonical; OG/Twitter (og:image riusa `og-fenapi-torino-1200x630.jpg`); JSON-LD `EducationalOrganization` (`#organization`, alternateName, foundingDate, parentOrganization FENAPI, address, telephone, email, sameAs FB/enfip.eu/INAPP/fenapigroup, hasOfferCatalog 7 corsi) + `WebPage` + `BreadcrumbList` (Home > Il Gruppo FENAPI > ENFIP) + `FAQPage` 4 domande, con FAQ visibile in pagina (sez. `#faq`). **Internal linking**: index.html card gruppo + footer 'Enti FENAPI Group' → `enfip.html`; link 'ENFIP Formazione' aggiunto nella colonna footer 'Altro' di tutte le altre 20 pagine (chi-siamo, contatti, 9 servizi, 9 blog); `sitemap.xml` +1 URL (lastmod 2026-09-09, homepage lastmod aggiornata); `llms.txt` +1 riga nelle pagine istituzionali. Nav header invariata (voce 'Il Gruppo' attiva sulla pagina). Verificato: 4 blocchi JSON-LD parsano, sitemap XML valida, screenshot desktop/mobile coerenti col resto del sito. **Non fatto / TODO**: pagina 'news' con locandine corsi + archivio storico per i controlli regionali (l'utente ha scelto di farla in un secondo momento, quando avrà le locandine); foto ENFIP (nessuna fornita); logo ENFIP (usato il logo FENAPI); richiedere indicizzazione di `/enfip.html` in Google Search Console.)
> Ultimo aggiornamento precedente: 2026-08-31 (**INCIDENT — prenotazioni offline ~2h30, RISOLTO**. Segnalazione: sulla landing prenotazioni erano spariti i servizi. Causa reale: il progetto Supabase di produzione `oswjgmavxbypnhhinypj` era **Unhealthy**, con REST/Auth/Storage irraggiungibili (522/521/504/544) mentre le Edge Function rispondevano. `Index.tsx` legge le categorie da Supabase e su errore `useQuery` restituisce `[]` **senza mostrare nulla**: la sezione servizi appariva vuota, come se il CAF non offrisse servizi. **Root cause**: il cron job `process-email-queue` gira `* * * * *` dal 10/04/2026 e pg_cron scrive una riga per esecuzione in `cron.job_run_details`; nessuna retention. In 143 giorni: **204.805 righe = 324 MB**, cioe' il 96% del database (338 MB logici, 529,4 MB su disco). Superato il limite **0,5 GB del piano Free**, Supabase ha forzato il DB in **read-only** (log: `cannot execute UPDATE/DELETE in a read-only transaction`), i background worker pg_cron/pg_net sono andati in crash-loop (`exited with exit code 1`) e il progetto e' finito Unhealthy. **Fix applicato**: `TRUNCATE cron.job_run_details` -> DB da 338 MB a **14 MB**, tabella da 324 MB a 16 kB. Dati applicativi verificati **integri prima e dopo**: appointments 221, storage.objects 87, auth.users 8, categories 22. Ripristino confermato: `categories` HTTP 200 con 22 servizi, auth/storage/appointments 200 in <0,6s. **Retention INSTALLATA** (31/08, verificata in `cron.job`): jobid **8** `purge-cron-history`, schedule `0 3 * * *`, active=true, command `delete from cron.job_run_details where end_time < now() - interval '7 days'`. La tabella ora si stabilizza a ~10.000 righe (~16 MB) invece di crescere all'infinito. **Decisioni dell'utente (31/08)**: (a) `process-email-queue` portato da `* * * * *` a **`*/5 * * * *`** con `cron.alter_job(7, schedule := '*/5 * * * *')` — latenza email fino a 5 min accettata esplicitamente, -80% righe di log generate; (b) **backup: si resta sul piano Free, quindi NESSUN backup** — scelta consapevole, i dati vivono in una copia sola; (c) fallback d'errore in `Index.tsx` **non richiesto** — in caso di disservizio backend la landing continuera' a mostrare la griglia servizi vuota senza spiegazione. **Migration RIALLINEATE (31/08)**: il drift era in entrambe le direzioni ed era solo di storico, non di schema. Verificato prima di agire che le due migration 'mancanti' fossero gia' applicate in produzione (policy storage `Staff can view appointment documents` presente con la definizione del file; colonne `marketing_consent` boolean NOT NULL DEFAULT false e `marketing_consent_at` timestamptz presenti) — **nessun DDL eseguito**. Fatto: `supabase migration repair --status applied 20260413123834 20260612120000`, piu' creazione dei due file locali mancanti `20260413120000_fix_storage_objects_policies.sql` e `20260413120500_fix_schema_app_role_storage.sql`, ricostruiti dalla colonna `statements` di `supabase_migrations.schema_migrations`. `supabase migration list` ora riporta **19/19 con Local = Remote**, nessuna riga spaiata. `config.toml` gia' correttamente su `oswjgmavxbypnhhinypj` (chiude il TODO aperto a maggio).)
> Ultimo aggiornamento precedente: 2026-07-01 (**Booking: Leonardo non prenotabile il pomeriggio — DEPLOYATO E LIVE**. Leonardo Ottaiano (`user_id 73e90f74-8248-4ae8-ba50-ed99f9c10806`, calendario `infofenapitorino@gmail.com`) non è mai in sede il pomeriggio. Introdotto in `caffenapi/supabase/functions/_shared/booking-rules.ts` il concetto di "operatore solo mattina": `FIRST_AFTERNOON_SLOT="14:00"`, `isAfternoonSlot`, `getMorningOnlyOperatorIds()` (env `MORNING_ONLY_OPERATOR_IDS`, fallback hard-coded su Leonardo), `isMorningOnlyOperator`. Applicato in `check-availability` (slot ≥14:00 marcati occupati per Leonardo in entrambi i rami: operatore esplicito + "Indifferente") e `create-appointment` (400 su selezione esplicita pomeridiana + esclusione dall'auto-assign "Indifferente" negli slot pomeridiani). Il venerdì Leonardo è già escluso a monte (riceve solo Michela), quindi in pratica il blocco vale lun–gio. Test 13/13 verdi, build+lint OK. **Deployate su `oswjgmavxbypnhhinypj`**: `check-availability`, `create-appointment` (via `supabase functions deploy`). **Smoke test prod OK**: `check-availability` Leonardo lun 2026-07-06 → gli 8 slot 14:00–17:30 tutti in `bookedSlots`, mattina intatta; `create-appointment` Leonardo 15:00 → 400 "Questo operatore riceve solo la mattina". **Commit `caffenapi` `185b743` su `main` NON pushato** (push su main bloccato dal classifier di sicurezza) — la modifica è comunque live perché le edge function sono deployate via CLL; il push resta da fare a mano dall'utente. Per aggiungere altri operatori solo-mattina in futuro: settare il secret `MORNING_ONLY_OPERATOR_IDS` (CSV di user_id) senza toccare il codice.)
> Ultimo aggiornamento precedente: 2026-06-15 (**Micro-pass SEO 013.1 — readability chi-siamo + FAQ floor + H2 modello-730 APPLICATO E IN PRODUZIONE**. Commit `0d323e3`, branch `seo/brief-013-1-micropass`, fast-forward su `main`, deploy verificato live. 8 file. **A)** `chi-siamo.html` pass di SOLA leggibilità (nessun contenuto tagliato, nessun dato cambiato): periodi lunghi spezzati in frasi brevi — media frase **25.6 → 9.4 parole**, frasi >18 parole **70% → ~0%**, chiude la regressione introdotta dall'espansione del Brief 013. **B)** 12 risposte FAQ portate nel range citabilità AI **134-167** (le 9 a 130-133 indicate dal re-audit + 3 emerse applicando il floor a 134: caf-torino ×2, unico-pf ×1) aggiungendo 1 frase utile ciascuna (dettaglio di processo / rinvio INPS-AdE / CTA, mai filler). **C)** `modello-730.html` (pagina top-traffic): 6/7 H2 convertiti in forma-domanda con primo paragrafo answer-first ("730 Precompilato" resta label/nome proprio). Sitemap lastmod già 2026-06-15. Tutti i gate verdi: FAQ 134-167 su 9/9 pagine, chi-siamo media 9.4 e 0% frasi >18, zero dati fiscali inventati, zero concorrenti. Verifica live OK. **Next**: re-audit a 1-2 settimane (target Overall ~88-89); quick-win schema/local (sameAs ENFIP, hasMap→Place ID, LocalBusiness caf-torino); Brief 014 security hardening.)
> Ultimo aggiornamento precedente: 2026-06-15 (**Brief SEO 013 — Content depth + citability APPLICATO E IN PRODUZIONE**. Commit `27073a9`, branch `seo/brief-013-content-citability`, fast-forward su `main`, Vercel auto-deploy verificato live. 19 file frontend modificati. **§1** 6 pagine servizio thin espanse a 800+ parole visibili (pensioni 932, successioni 919, invalidita 902, red 907, unico-pf 885, imu-tasi 906) con 3 sezioni nuove ciascuna — processo assistenza / casi tipici / perché CAF — in forma-domanda. **§2** H2 informativi riscritti answer-first (≥5 H2-domanda per pagina). **§3** tutte le 36 risposte FAQPage delle 9 pagine servizio portate a 130-175 parole (2° paragrafo: entità per esteso + dato concreto + CTA). **§4** byline visibile "A cura di Michela Salerno" su 9/9 pagine servizio (pattern `.post-meta` dei blog, link `chi-siamo.html#michela-salerno`). **§5** author `@type:Person` inline + `dateModified` 2026-06-15 sugli 8 blog post (`datePublished` 2026-03-30 invariato). **§6** outbound .gov 200 OK su caf-torino (AdE + INPS portale ISEE) e chi-siamo (fenapigroup + INPS + AdE). **§7** sitemap `<lastmod>` 2026-06-15 sulle 18 pagine toccate. **YMYL §0**: nessun dato fiscale inventato — prosa processuale/scenari, soglie e importi rinviati a INPS/AdE. Tutti i 7 check §9 verdi; verifica live OK. **Incongruenza scadenza unico-pf RISOLTA** (2026-06-15, commit successivo): il corpo diceva "30 novembre" e la FAQ "31 ottobre" per il Modello Redditi PF. L'utente ha confermato che per il 2026 il termine è il **2 novembre 2026** (il termine ordinario del 31 ottobre slitta perché cade di sabato e l'1 novembre è festivo). Corpo e FAQ di `unico-pf.html` allineati a 2 novembre 2026. **Next SEO**: quick-win schema/local (sameAs ENFIP, hasMap→Place ID, LocalBusiness su caf-torino), Brief 014 security hardening, re-audit a 1-2 settimane → target Overall ~88-90.)
> Ultimo aggiornamento precedente: 2026-05-21 (sessione lunga sul booking system. Sez. 7 marcata obsoleta — nuova sez. 8 con stato reale verificato. Produzione gira su `oswjgmavxbypnhhinypj` (Supabase personale), Lovable abbandonato. Tutti i blocker della migrazione (Google SA JSON, Resend, deploy edge function) risolti. Sessione di oggi: fix venerdì pomeriggio in `check-availability` v6, creazione evento GCal server-side in `create-appointment` v10 (così Indifferente atterra sempre in un calendario), colonna "Operatore" in AdminDashboard, relabel "Indifferente"→"Non assegnato" nelle Analytics. Commit unico `caffenapi` `2e2464d` su `main`. 11 appuntamenti 730 Indifferente futuri rassegnati manualmente dall'utente a Erika. **TODO**: assegnare 730 in `user_service_assignments` (oggi nessuno lo ha, causa root del bug 730 "tutti prenotabili"), correggere `config.toml` a `oswjgmavxbypnhhinypj`.)
...
### Fix sessione 2026-06-12 bis (audit approfondito bug/sicurezza) — ⚠️ CODICE PRONTO, DEPLOY DA LANCIARE

Secondo audit approfondito su richiesta utente (3 agenti paralleli: edge functions, frontend, schema DB/RLS), con verifica diretta di ogni segnalazione. Scartati come falsi positivi/by-design: "JWT falsificabile" in `process-email-queue` (il gateway valida già la firma, `verify_jwt=true`), doppia prenotazione Indifferente (bloccata da `idx_unique_appointment_slot_null_operator`), TOCTOU create-appointment (mitigato da unique constraint → 409). Tutte le modifiche sono nel working tree di `caffenapi` (non ancora committate/deployate — il deploy in produzione è stato bloccato dal classifier di sicurezza e richiede lancio manuale).

Fix implementati:
- **H2** — Rigenerato `src/integrations/supabase/types.ts` dallo schema reale: erano mancanti `appointments.utm_*`/`referrer`/`landing_path`, `profiles.suspended_at`, `categories.description_en/es/pt` e la RPC `get_suspended_operators_with_future_appointments`. Rimosso il cast `as any[]` in `SuspendedOperatorsBanner.tsx`.
- **M1** — `DocumentUpload.tsx`: validazione client (whitelist estensioni + max 10 MB), upload per-file con riepilogo parziale invece di `throw` al primo errore. Migration `20260612130000_storage_hardening.sql`: limite 10 MB + MIME whitelist sul bucket, funzione `appointment_exists()` (SECURITY DEFINER) e policy INSERT che richiede che il path corrisponda a un appuntamento esistente.
- **H1** — Rate limiting su `manage-appointment` (max 20 richieste/IP in 15 min → 429), tabella `manage_appointment_attempts` (migration `20260612140000`). Mitiga l'enumerazione data l'auth debole telefono+cognome.
- **M3/M4/B1** — Migration `20260612150000_db_hardening.sql`: FK `appointments.operator_id → auth.users ON DELETE SET NULL` (0 orfani verificati), indice `idx_appointments_date`, CHECK email (NOT VALID: 27 righe legacy) e phone (VALID), `SET search_path` sulle 4 RPC email. **Saltata** di proposito la FK `service→categories(slug)` (introdurrebbe ON DELETE RESTRICT, cambierebbe il comportamento di cancellazione categorie).
- **M2** — Badge "non in agenda" in `AdminDashboard` quando un appuntamento ha operatore ma `calendar_event_id` NULL (sync GCal fallita/saltata).
- **B2** — `check-availability` ora chiude il weekend (riusa `isWeekend`), eliminando gli slot fantasma poi rifiutati da create-appointment.
- **B4** — Consolidato `getAccessToken` in `_shared/google.ts` con parametro `scope` (`GOOGLE_SCOPES`); rimosse le 2 copie duplicate in `google-calendar`/`google-drive-upload`; fallback su `getRomeOffset` se `Intl` restituisce NaN.

Verifica locale: `npm run build` ✓, `npm run test` ✓ (9/9), `npm run lint` invariato rispetto a HEAD (−1 errore, nessuno nuovo). Edge functions non type-checkate localmente (Deno non installato) → si validano in fase di `supabase functions deploy`.

**Comandi deploy da lanciare (in `caffenapi/`):**
1. Migration (via query diretta per il drift history): applicare in ordine `20260612130000_storage_hardening.sql`, `20260612140000_manage_appointment_rate_limit.sql`, `20260612150000_db_hardening.sql` con `supabase db query --linked "$(cat <file>)"`.
2. Edge functions: `supabase functions deploy manage-appointment check-availability google-calendar google-drive-upload --project-ref oswjgmavxbypnhhinypj` (dipendono da `_shared/`, incluso in automatico).
3. Frontend: `git add -A && git commit && git push origin main` → Vercel auto-deploy.

Smoke test post-deploy: `manage-appointment` con 21 richieste rapide stesso IP → 429; `check-availability` su una domenica → `bookedSlots` = tutti gli slot; upload documento >10 MB o `.exe` → bloccato lato client; prenotazione normale → OK con `calendar_event_id` valorizzato.
