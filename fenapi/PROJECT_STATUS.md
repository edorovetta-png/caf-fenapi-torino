...
> Ultimo aggiornamento: 2026-04-11 (brief SEO 007 applicato: FAQPage JSON-LD su tutte e 8 le pagine servizio — modello-730, modello-isee, imu-tasi, unico-pf, red, pensioni, successioni, invalidita — 4 Q&A ciascuna, inserite dopo il BreadcrumbList schema. Creato `frontend/llms.txt` (llmstxt.org standard) con H1 + blockquote + sezioni Contatti/Servizi/Istituzionali/Blog/Risorse/Optional. Aggiunto commento `llms.txt` a `robots.txt` preservando direttive esistenti. Validazione JSON-LD locale: tutti 8 file = 3 blocks OK (LocalBusiness/Service + BreadcrumbList + FAQPage). Nessuna modifica a layout/DOM/testi editoriali/schema esistenti. Target impact: AI Readiness 58→~72, Schema 85→~92. Da fare post-merge: re-audit SEO, Rich Results Test, verifica curl llms.txt su preview.)
